...
function AddCustomer() {
  ...
  const [formData, setFormData] = useState({
    shipping_stops: [{ customer_id: "", sequence: "" }],
    // sequence: "",
    // status: "รอการอนุมัติ",
  });
  ...
  const fetchData = async () => {
    try {
      ...
      // console.log(CustomersResponse.data.data); // ตรวจสอบข้อมูลที่ได้
      setCustomers(CustomersResponse.data.data);

      const RoutetemplatesResponse = await RoutetemplatesService.getAll();
      // console.log(RoutetemplatesResponse.data.data || []); // ตรวจสอบข้อมูลที่ได้
...
const filteredCustomers = routetemplate
  .flatMap(
    (route) =>
      route.customers.map((cust) => ({ ...cust, routeName: route.name })) // Add route name for reference
  )
  .filter((cust) =>
 ...
 const handleCheckboxChange = (cust) => {
   setSelectedCustomers((prevState) => {
     ...
     // Update formData.shipping_stops
     setFormData((prev) => ({
       ...prev,
       shipping_stops: updatedSelectedCustomers.map((item) => ({
         customer_id: item._id, // Ensure using _id here
         sequence: item.sequence,
       })),
     }));

     console.log(updatedSelectedCustomers);
  ...
  const handleSubmit = async (event) => {
    ...
    // ตรวจสอบว่ามีการเลือก shipping_stops หรือไม่
    ...
    try {
      // ส่งข้อมูล shipping_stops ที่เป็น Array ไปยัง Ship_worksService.addCustomer
      const response = await Ship_worksService.addCustomer(id, {
        shipping_stops: selectedCustomers.map((cust) => ({
      ...
      if (response.status === 201) {
        ...
        setFormData({
          shipping_stops: [], // รีเซ็ต shipping_stops ให้เป็น Array ว่าง
        });
        setSelectedCustomers([]); // รีเซ็ต selectedCustomers
      }
    } catch (error) {
